...
class PluginFlyvemdmProfile extends CommonTestCase {
   public function testPre_PurgeItem() {
      ...
      // pre_purgeItem should succeed
      $profile->input = $profile->fields;
      plugin_flyvemdm_hook_pre_profile_purge($profile);
      $this->array($profile->input);

      $config = \Config::getConfigurationValues('flyvemdm', ['agent_profiles_id', 'guest_profiles_id']);
      $profile = new \Profile();
      $profile->getFromDB($config['agent_profiles_id']);
      $profile->input = $profile->fields;
      plugin_flyvemdm_hook_pre_profile_purge($profile);
      $this->boolean($profile->input)->isFalse();
